...
instance Eq a => Eq (List a) where
  ...
  Empty         == Empty         = True
  LNode _ Empty == Empty = False
  LNode a Empty == LNode b Empty =  a == b
  LNode a al    == LNode b bl    = (a == b && al == bl)
  _             == _            = False
  -- LNode a al    == LNode b Empty = False
...
simplify (RationalNumber a b) = RationalNumber (div a g) (div b g)
  where g = myGcd (abs a) (abs b)

-- Implementation I wrote in set 2b
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 s = s
  stepMany i s = stepMany (i-1) (step s)
